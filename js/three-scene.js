...
(function () {
  ...
  /* ══════════════════════════════════════════════════════════════
     COSMIC SINGULARITY — Premium Scroll Transition
     ─────────────────────────────────────────────────────────────
     When #skills enters viewport → gravity spiral collapse, star
     field fades in, nebula blooms, sphere becomes glowing energy
     core. Fully reversible on scroll up.
  ══════════════════════════════════════════════════════════════ */
  ...
  const starMat = new THREE.ShaderMaterial({
    ...
    fragmentShader: `
      ...
      void main() {
        ...
        vec2 uv2 = vWorldPos.xy * 4.2 - uTime * 0.18;
        float e1 = fbm(uv1);
        ...
        vec3 finalCol = mix(eclipseCol, starCol, uT);
        gl_FragColor  = vec4(finalCol, 1.0);
      }
    `,
    side: THREE.FrontSide,
  });
  sphereMesh.material = starMat;
  ...
  function buildStarField() {
    ...
    const sizes = new Float32Array(allN);
    ...
    const mat = new THREE.ShaderMaterial({
      ...
      vertexShader: `
        ...
        uniform float uT;
        void main() {
          float twinkle = 0.65 + 0.35 * sin(uTime * 1.8 + aSeed * 12.566);
          gl_PointSize = aSize * twinkle * uT * 2.0;
  ...
  const { pts: starPoints, mat: starPointsMat } = buildStarField();

  /* ─── Layer D: Nebula Planes (purple + cyan) ─── */
  function buildNebulaMat(col1, col2) {
    return new THREE.ShaderMaterial({
      ...
      fragmentShader: `
        ...
        void main() {
          vec2 p = vWP.xy * 0.004 + uTime * 0.003;
          float n1 = fbm(p);
          float n2 = fbm(p + n1*0.8 + vec2(3.2,1.7));
          float n  = clamp(n2 * 1.5 - 0.35, 0.0, 1.0);
          vec3 col = mix(uC1, uC2, n) * n;
          float radFade = 1.0 - clamp(length(vUv - 0.5)*1.8, 0.0, 1.0);
  ...
  /* ─── Spark update helper ─── */
  let sparkFrame = 0;
  const SR_eff = CFG.sphereR;

  function updateSparks(t, pulse) {
    if (t < 0.5) {
    ...
    sparkFrame++;
    const spawnT = (t - 0.5) * 2.0;

    for (let i = 0; i < SPARK_N; i++) {
      sparkEscT[i]++;
      const phase = sparkEscT[i] % 320;
      let r, ex, ey, ez;
  ...
  function masterAnimate() {
    ...
    /* Background: blend user bgColor → deep space black */
    const deepBlack = new THREE.Color(0x000005);
    bgCO.lerp(deepBlack, sT);
    ...
    // Gravity + spiral collapse — scaled by scroll velocity so fast scrolls
    // snap particles in immediately while slow scrolls feel elegant
    ...
    // If collapsed enough, kill draw entirely so nothing lingers
    if (sT > 0.75) {
      ...
      for (let i = 0; i < particleCount; i++) {
        ...
        pvx[i] += (avgVX - pvx[i]) * alignK * alignSuppress;
        pvy[i] += (avgVY - pvy[i]) * alignK * 0.5 * alignSuppress;
